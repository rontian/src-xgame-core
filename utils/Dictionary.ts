...
clazz('Dictionary');
export class Dictionary<TK, TV> extends XObject {
    private limit = 0;
    ...
    public shift(): { key: TK; value: TV } {
        if (this.length > 0) {
            const item = { key: this.m_keys.shift(), value: this.m_values.shift() };
    ...
    public pop(): { key: TK; value: TV } {
        if (this.length > 0) {
            const item = { key: this.m_keys.pop(), value: this.m_values.pop() };
            this.mapdelete(item.key);
            return item;
        }
        return null;
    ...
    public containsKey(key: TK): boolean {
        ...
        const indexOf: number = this.indexOfKey(key);
        if (indexOf >= 0) {
            return true;
        }
        return false;
    ...
    public containsValue(value: TV): boolean {
        const indexOf: number = this.indexOfValue(value);
        if (indexOf >= 0) {
            return true;
        }
        return false;
    ...
    public allocf(key: TK, defaultFactory: () => TV): TV {
        ...
        const defaultValue: TV = defaultFactory();
    ...
    public removeByValue(value: TV): TK {
        ...
        if (indexOf >= 0) {
            ...
            const key = this.m_keys.splice(indexOf, 1)[0];
            this.mapdelete(key);
            return key;
        }
        return null;
    ...
    public clear(recycle?: (item: TV) => void): void {
        if (recycle) {
            const len = this.m_values.length;
    ...
    public random(): TV {
        const index: number = (Math.random() * this.keys.length) << 0;
