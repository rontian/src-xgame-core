...
export class MathParser extends XObject {
        ...
        while (this.pos < this.expression.length) {
            if (this.isOperator()) {
                if (this.isSign() && expected[6] == 1) {
                    const mintoken: Token = new Token(this.TNUMBER, 0, 0, -1);
                ...
                const token: Token = new Token(this.TNUMBER, 0, 0, this.tokennumber);
                ...
                const consttoken: Token = new Token(this.TNUMBER, 0, 0, this.tokennumber);
                ...
                const vartoken: Token = new Token(this.TVAR, this.tokenindex, 0, 0);
    ...
    public simplify(parsedexpression: Token[]): Token[] {
        ...
        let n2: Token;
        let f: Function;
        const L: number = parsedexpression.length;
        let item: Token;
        let i = 0;
        for (i = 0; i < L; i++) {
            item = parsedexpression[i];
            const type_: number = item.type_;
    ...
    public eval(pexpression: Token[], values: number[]): number {
        ...
        let n2: number;
        let f: Function;
        const L: number = pexpression.length;
        let item: Token;
        let i = 0;
        for (i = 0; i < L; i++) {
            item = pexpression[i];
    ...
    private addfunc(tokenstack: Token[], operstack: Token[], type_: number): void {
        const operator: Token = new Token(type_, this.tokenindex, this.tokenprio + this.tmpprio, 0);
        while (operstack.length > 0) {
    ...
    private isNumber(): boolean {
        ...
        let str = '';
        while (this.pos < this.expression.length) {
            const code: number = this.expression.charCodeAt(this.pos);
    ...
    private isSign(): boolean {
        const code: number = this.expression.charCodeAt(this.pos - 1);
        if (code == 45) {
            return true;
        }
        return false;
    ...
    private isFunc1(): boolean {
        let str: string;
        for (let i = 0; i < this.funcs1.length; i++) {
            const L: number = this.funcs1[i].length;
    ...
    private isFunc2(): boolean {
        let str: string;
        for (let i = 0; i < this.funcs2.length; i++) {
            const L: number = this.funcs2[i].length;
            str = this.expression.substr(this.pos, L);
