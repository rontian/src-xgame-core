...
export class SignalBase extends XObject {
    ...
    public add(listener: Function, thisObject: any = null, priority = 0): void {
        if (this.nodes.has(listener)) {
            return;
        }
        const node: ListenerNode = this.listenerNodePool.get();
    ...
    public addOnce(listener: Function, thisObject: any = null, priority = 0): void {
        if (this.nodes.has(listener)) {
            return;
        }
        const node: ListenerNode = this.listenerNodePool.get();
    ...
    public remove(listener: Function): void {
        const node: ListenerNode = this.nodes.get(listener);
        if (node) {
            if (this.sortEnable) {
    ...
    public removeAll(): void {
        while (this.head) {
            const node: ListenerNode = this.head;
            this.head = this.head.next;
    ...
    private sortEnable = false;

    private invalidateSort = false;
