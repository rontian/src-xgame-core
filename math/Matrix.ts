...
export class Matrix extends XObject {
    ...
    $invertInto(target: Matrix): void {
        ...
        const k = (target.a = d * determinant);
    ...
    public deltaTransformPoint(point: Point): Point {
        const self = this;
        const x = self.a * point.x + self.c * point.y;
        const y = self.b * point.x + self.d * point.y;
        return new Point(x, y);
    }
    ...
    $getScaleX(): number {
        ...
        const result = Math.sqrt(m.a * m.a + m.b * m.b);
    ...
    $getScaleY(): number {
        ...
        const result = Math.sqrt(m.c * m.c + m.d * m.d);
        return this.getDeterminant() < 0 ? -result : result;
    }
    ...
    $updateScaleAndRotation(scaleX: number, scaleY: number, skewX: number, skewY: number) {
        ...
        const u = Math.cos(skewX);
        const v = Math.sin(skewX);
        if (skewX == skewY) {
            this.a = u * scaleX;
            this.b = v * scaleX;
        } else {
