...
import { IDateInfo } from './DataStructs';
import { IDateTimeManager } from './IDateTimeManager';

export const DATE_HOUR_TIME = 3600;
export const DATE_DAY_TIME = 86400;
/**
 * 日期，时间戳管理器
 */
@facade(IDateTimeManager)
@clazz('DateTimeManager')
export class DateTimeManager extends Singleton implements IDateTimeManager {
    public diffTimestamp = 0;

    //默认中国时区GMT+8
    public timeZone = 28800;
     ...
     * @param timeZone
     */
    public setNowTimestamp(timestamp: number, timeZone = 0): void {
    ...
    public formatSeconds(formatString: string, time: number): string {
        ...
        for (const k in o) {
            if (new RegExp('(' + k + ')').test(formatString)) {
                formatString = formatString.replace(RegExp.$1, RegExp.$1.length == 1 ? <string>o[k] : ('' + o[k]).length == 1 ? '0' + o[k] : o[k]);
            }
        }
        return formatString;
    }
    ...
    public getStringFromDate(info_or_formatString?: IDateInfo | string, formatString = 'yyyy/MM/dd hh:mm:ss'): string {
        let info: IDateInfo;
    ...
    public getDateFromString(text: string, timestamp?: number): IDateInfo {
        ...
        const info = this.getDateInfo(timestamp);
        if (list.length == 1) {
            if (list[0].indexOf('/') >= 0) {
                dateString = list[0];
        ...
        const time_list = timeString.split(':');
    ...
    public getTimestampFromDate(info: IDateInfo, timestamp?: number): number {
        const now = this.getDateInfo(timestamp);
    ...
    public getTimestampWithWeek(week: number, timestamp?: number, fixed?: IDateInfo): number {
        ...
        const diff_days = week - info.week;
    ...
    public getTimestampWithNextWeek(week: number, timestamp?: number, fixed?: IDateInfo): number {
        const time = this.getTimestampWithWeek(week, timestamp, fixed);
    ...
    public getLastDayTimestampWithMonth(timestamp?: number, fixed?: IDateInfo): number {
        const time = this.getFirstDayTimestampWithNextMonth(timestamp, fixed);
    ...
    public getTimestampWithTomorrow(timestamp?: number, fixed?: IDateInfo): number {
        fixed = this.fixedDateInfo(fixed);
        const info = this.getDateInfo(timestamp, DATE_DAY_TIME);
        info.ms = 0;
        if (fixed) {
            if (fixed.hour != undefined) {
                info.hour = fixed.hour;
    ...
    public getDayID(timestamp?: number): number {
        timestamp = timestamp || this.getNowTimestamp();
        const time = timestamp + this.timeZone;
