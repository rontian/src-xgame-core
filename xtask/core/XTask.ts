...
import { IXTaskDelegate } from '../interfaces/IXTaskDelegate';
...
@facade(IXTask)
@clazz('XTask')
export class XTask extends Locker implements IXTask {
    public name: string;
    ...
    protected countCompleted = 0;

    protected walkIndex = 0;

    protected currentChildComplete = 0;

    protected currentComplete = 0;
    ...
    public constructor(mode: XTaskMode = XTaskMode.Parallel, life = 0) {
        super();
    ...
    public addTask(task: any, mode: XTaskMode = XTaskMode.Parallel, life = 0): any {
        this.simple(() => {
            let item: XTask = task;
    ...
    public removeTask(task: any): void {
        this.simple(() => {
            ...
            if (this.children.containsKey(hashCode)) {
                const value = this.children.remove(hashCode);
    ...
    public mainLoop(): void {
        ...
        if (this.state == XTaskState.SelfCompleted && this.childCount > 0) {
            ...
            for (let i = this.walkIndex; i < this.childCount; i++) {
                const task = this.children.indexValue(i);
                this.walkTask(task);
    ...
    public complete(): void {
        ...
        this.countCompleted++;
        const parent = <XTask>this.parent;
    ...
    protected walkTask(task: XTask): void {
        const isMainLocked = this.isMain && XTaskManager.Instance().mainTask.isLockTask(task);
        let removed = false;
        switch (task.state) {
    ...
    protected CheckComplete(): void {
        if (this.isSelfCompleteSucceed && this.childCount > 0) {
            if (this.mode == XTaskMode.Sequence || this.mode == XTaskMode.RacingSequence) {
                const last = this.children.last().value;
    ...
    private checkResets(): void {
        while (this.$forResets.length) {
            const hashCode = this.$forResets.shift();
            if (this.children.containsKey(hashCode)) {
                const value = this.children.get(hashCode);
                value.reset();
            }
        }
    }
    ...
    private checkRemoves(): void {
        while (this.$forRemoves.length) {
            const hashCode = this.$forRemoves.shift();
