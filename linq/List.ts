...
import { composeComparers, keyComparer, negate } from './helpers';
...
export class List<T> extends XObject {
     ...
     */
    public groupBy<TResult>(grouper: (key: T) => string | number, mapper: (element: T) => TResult = (val) => val as any as TResult): { [key: string]: TResult[] } {
    ...
    public toDictionary<TKey, TValue>(key: (key: T) => TKey, value?: (value: T) => TValue): List<{ Key: TKey; Value: T | TValue }> {
        return this.aggregate((dicc, v, i) => {
            const _key = this.select(key).elementAt(i);
            const _value = value ? this.select(value).elementAt(i) : v;
            (<any>dicc)[_key.toString()] = _value;
            dicc.add({
                Key: _key,
                Value: _value,
            });
    ...
    public toSet() {
        ...
        for (const x of this._elements) result.add(x);
